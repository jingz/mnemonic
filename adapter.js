...
Adapter = (function() {
    ...
    function Adapter(config) {
        config = config || ENV.WEB_DB_CONFIG;
    ...
    Adapter.prototype.execute = function(sql, params, cb_rec, cb_err) {
        this.connection.transaction(function  (tx) {
            tx.executeSql(sql, params, function (_tx, results) {
                // console.log(sql, params); // debug
                if(typeof cb_rec == 'function'){
                    try {
                        // if inserted successful
                        // there will be an insertId property
                        // in results set
                        // otherwise InvalidAccessError: DOM Exception 15
                        cb_rec(results.insertId);
                    } catch(e){
                        if(results){
                            var records = [];
